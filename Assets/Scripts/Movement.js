...
function tracking(){
	...
	//ganglion cells status
	for(var g : Ganglion in Ganglions){
		if(g.active == 1){
			var dis_y = 0.0;
			var dis_z = 0.0;
...
function showResult(){
	for(var v : Vector3 in positions){
		if(v.z>0){
			var r_showPixel = Instantiate(cel,Vector3(-3,-v.y*10,-(v.z-0.325)*10),Quaternion.identity);
			r_showPixel.transform.parent = GameObject.Find("Result").transform;
			r_showPixel.renderer.material.color.a = 0.2;
			r_showPixel.tag = "Result";
		}
		if(v.z<0){
			var l_showPixel = Instantiate(cel,Vector3(-3,-v.y*10,-(v.z+0.325)*10),Quaternion.identity);
			l_showPixel.transform.parent = GameObject.Find("Result").transform;
			l_showPixel.renderer.material.color.a = 0.2;
			l_showPixel.tag = "Result";
		}
